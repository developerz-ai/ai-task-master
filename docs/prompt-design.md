...
Role map onto aitm today: the **Worker is the task-level coordinator** — it holds the edit tools
itself, decides whether to do the work inline (small/cohesive) or fan out leaf editors (genuinely
large work), and owns the submit. The **editors are the single-level leaves** (their tool set
already has `explore`/`memory` stripped, so they cannot nest). The **Orchestrator** is harness-driven
prose only (commit message + PR body). So the "only the coordinator holds the spawn tool" rule already
holds structurally — the prompts below make it explicit and add the inline/fan-out decision plus the
right-sizing judgment.
...
- a very large file is still ONE leaf (one path = one owner); never two leaves on one path, they
  clobber each other.
- every leaf MUST be independent — no leaf may depend on another leaf's output. Dependencies
  within a task mean you split on the wrong axis; do those inline instead.
...
Rationale: encodes the inline-vs-fanout decision + the >~500 LOC / multi-area sizing rule +
right-size + parallel-independence + "only you spawn" + "you own verification", in the one place the
model actually decides them. Bias is toward action: survey only what you will change (never framework
internals), then edit. Keeps the compaction line from today's prefix; drops the two-layer/SDK jargon
(that is harness concern, not model judgment).
