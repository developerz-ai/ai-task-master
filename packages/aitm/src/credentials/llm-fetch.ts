...
function isDispatcher(value: unknown): value is KeepAliveDispatcher {
...
// Build the keep-alive fetch, or undefined when the runtime already pools connections (Bun/Deno) or
// undici is unavailable. Undefined signals `providerSettings` to omit `fetch` and keep the
// provider's default transport, so the non-Node request path stays byte-identical.
export async function createLlmFetch(
  ...
  if (!isDispatcher(dispatcher)) return undefined;

  const baseFetch = options.baseFetch ?? globalThis.fetch;
  const keepAliveFetch: typeof fetch = (input, init) => baseFetch(input, { ...init, dispatcher });
