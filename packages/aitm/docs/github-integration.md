...
GraphQL queries are batched per PR (threads + comments fetched in one request). CI status polling uses exponential backoff: 1s start, doubling, 60s cap. A single unparseable or failed `gh pr checks` read — a network blip, a truncated stdout — does not abandon the wait: the poll rides through up to `CHECKS_MAX_CONSECUTIVE_FAILURES` (5) consecutive bad reads on the same backoff before surfacing the failure, and any good read resets the count. A checkless PR (gh exits non-zero with "no checks reported on the '<branch>' branch" and an empty stdout) is treated as an empty check set that becomes mergeable after the empty grace, not a hard error.
